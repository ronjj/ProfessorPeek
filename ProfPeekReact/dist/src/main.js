...
function showHoverText() {
    ...
    classListing.appendChild(hoverText);

    // Function to handle the mouseover event
    function handleMouseOver(event) {
        hoverText.style.left = `${event.pageX + 10}px`; // Position near the cursor
        hoverText.style.top = `${event.pageY + 10}px`;  // Position near the cursor
    ...
    // Attach event listeners to all elements with the class 'rmp-link'
    const rmpLinks = document.querySelectorAll('a.rmp-link');
    rmpLinks.forEach(link => {
